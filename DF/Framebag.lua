...
local function ItemCreate(name, parent)
	...
	button.bg:SetTexture[[Interface\AddOns\DF\Textures\BackSlot]]
	button.bg:SetAlpha(.85)
...
local function PrepareBank(frame)
	...
	frame.bank:SetPoint('CENTER', -400, 120)
	frame.bank:SetScript('OnMouseDown', function() this:StartMoving() end)
	frame.bank:SetScript('OnMouseUp', function() this:StopMovingOrSizing() end)
	frame.bank:SetToplevel()
	frame.bank:EnableMouse()
	frame.bank:SetMovable()
	frame.bank:SetClampedToScreen()
...
SUCC_bag:SetPoint('CENTER', UIParent, 495, -55)
...
for i = 1, NUM_BAG_SLOTS, 1 do
	...
	SUCC_bag.slotFrame.slot[i]:SetNormalTexture('Interface\\AddOns\\DF\\Textures\\Slotbag')
...
SUCC_bag.keyring:SetPoint('BOTTOMRIGHT', UIParent, -90, 45)
